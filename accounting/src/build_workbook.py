...
def main():
    ...
    header(ws, 4, ["ID", "Date", "Bank / card", "Description", "Amount", "Code", "Account",
                   "Confidence", "Why it was coded this way"],
           [8, 12, 23, 62, 13, 8, 36, 12, 62])
    r = 5
    for t in led:
        ws.cell(row=r, column=1, value=t["txn_id"]); ws.cell(row=r, column=2, value=t["date"])
        ws.cell(row=r, column=3, value=raw_src[t["txn_id"]]); ws.cell(row=r, column=4, value=t["description"])
        ...
        if t["confidence"] == "review":
            cf.fill = WARN
        ws.cell(row=r, column=9, value=t["note"])
        r += 1
    ws.auto_filter.ref = f"A4:I{r-1}"

    # ───────────────────────── Customer Payments ─────────────────────────
    ws = wb.create_sheet("Customer Payments")
    title(ws, "Every Customer Payment",
          "From HighLevel. The event date is parsed from the invoice name, and decides which period the revenue lands in.")
    header(ws, 4, ["Paid on", "Amount", "Customer", "Invoice", "Event date", "Earned or deferred"],
           [13, 13, 30, 46, 13, 20])
    r = 5
    cutoff = st["period"]["end"]
    for pmt in sorted(ghl, key=lambda x: (x["paid_date"] or "")):
        ws.cell(row=r, column=1, value=pmt["paid_date"])
        c = ws.cell(row=r, column=2, value=pmt["amount"]); c.number_format = M
        ws.cell(row=r, column=3, value=pmt["contact"])
        ws.cell(row=r, column=4, value=pmt["invoice"])
        ws.cell(row=r, column=5, value=pmt["event_date"] or "")
        if not pmt["event_date"]:
            state, fill = "no date on invoice", WARN
        elif pmt["event_date"] > cutoff:
            state, fill = "DEFERRED", WARN
        else:
            state, fill = "earned", None
        sc = ws.cell(row=r, column=6, value=state)
        if fill:
            sc.fill = fill
        r += 1
    ws.auto_filter.ref = f"A4:F{r-1}"
    ws.cell(row=r, column=1, value="TOTAL").font = Font(bold=True)
    c = ws.cell(row=r, column=2, value=round(sum(p["amount"] for p in ghl), 2))
    c.number_format = M; c.font = Font(bold=True); c.fill = TOTAL

    # ───────────────────────── Month by Month ─────────────────────────
    ws = wb.create_sheet("Month by Month")
    title(ws, "Month by Month",
          "Revenue sits in the month the event happened. Cash sits in the month it moved. "
          "Owner capital is kept in its own column so it does not read as revenue, and cash out includes fleet purchases.")
    header(ws, 4, ["Month", "Revenue earned (by event)", "Customer cash in", "Cash out (incl. fleet)",
                   "Net before owner", "Owner in / out", "Transactions"], [12, 24, 17, 18, 15, 16, 14])
    earned = collections.defaultdict(float)
    for pmt in ghl:
        if pmt["event_date"] and pmt["event_date"] <= cutoff:
            earned[pmt["event_date"][:7]] += pmt["amount"]
    # Owner capital is not revenue and not an operating cost, so it gets its own column
    # rather than inflating the month it landed in.
    cin = collections.defaultdict(float); cout = collections.defaultdict(float)
    owner = collections.defaultdict(float); cnt = collections.Counter()
    for t in led:
        if t["account_code"] == "9010":
            continue
        k = t["date"][:7]
        cnt[k] += 1
        if t["account_code"] in ("3010", "3020"):
            owner[k] += t["amount"]
        elif t["amount"] > 0:
            cin[k] += t["amount"]
        else:
            cout[k] += t["amount"]
    r = 5
    for k in sorted(set(earned) | set(cin) | set(cout) | set(owner)):
        ws.cell(row=r, column=1, value=k)
        for col, val in ((2, earned.get(k, 0)), (3, cin.get(k, 0)), (4, cout.get(k, 0)),
                         (5, cin.get(k, 0) + cout.get(k, 0)), (6, owner.get(k, 0))):
            c = ws.cell(row=r, column=col, value=round(val, 2)); c.number_format = M
        ws.cell(row=r, column=7, value=cnt.get(k, 0))
        r += 1
    for col, val in ((2, sum(earned.values())), (3, sum(cin.values())), (4, sum(cout.values())),
                     (5, sum(cin.values()) + sum(cout.values())), (6, sum(owner.values()))):
    ...
    for a in coa:
        for i, k in enumerate(["code", "account", "type", "subtype", "statement", "notes"], 1):
            ws.cell(row=r, column=i, value=a[k]).font = Font(size=9)
        if r % 2 == 0:
            for col in range(1, 7):
                ws.cell(row=r, column=col).fill = BAND
        r += 1
